...
customMatchController.startStream = async (req, res) => {
  try {
    ...
    // Set stream status with video format preferences
    match.stream = {
      isLive: true,
      roomId: roomId,
      startedBy: req.user._id,
      ...
      recordingStatus: 'recording',
      activeRecordingId: recordingId,
      preferredFormats: ['mp4', 'webm'], // Preferred formats for better compatibility
      maxDuration: 60 * 60 * 1000 // 1 hour max duration
    };

    // Create initial pastStream entry with video format info
    match.pastStreams.push({
      roomId: roomId,
      startedBy: req.user._id,
      startedAt: new Date(),
...
// Updated uploadRecording function for customMatch-controller.js
// customMatchController.uploadRecording = async (req, res) => {
//   try {
//     const { matchId } = req.params;
//     const { recordingId } = req.body;
...
//         req.file.path,
//         `recording_${matchId}_${Date.now()}.webm`,
//         '/cricket-streams'
//       );
...
//       console.log(`Recording uploaded to ImageKit for match ${matchId}: ${recordingUrl}`);
//       console.log(`ImageKit File ID: ${uploadResult.fileId}`);
//       console.log(`PastStreams count after upload: ${match.pastStreams.length}`);
  ...
  try {
    ...
    try {
      ...
      const generateImageKitVideoUrl = (filePath) => {
        // Replace with your actual ImageKit ID from dashboard
        ...
        // Construct URL that ends with /ik-video.mp4 for video processing
        return `${imageKitEndpoint}/${cleanPath}/ik-video.mp4`;
      };

      // Use it to generate the proper video URL
...
///////////////////////////////////
// Add to your customMatch-controller.js
