  ...
  useEffect(() => {
    ...
    const fetchCommentary = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3026/api/matches/${matchId}/commentary`, {
    ...
    const socket = io('http://localhost:3026');
    ...
    <div className="bg-white rounded-lg shadow-lg p-4 max-h-96 overflow-y-auto">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mt-4 p-2 bg-gray-100 rounded text-xs">
          <p>Commentary items: {commentary.length}</p>
          <p>Match ID: {matchId}</p>
          <p>Is Live: {isLive.toString()}</p>
        </div>
      )}
    </div>
  );
