...
const MatchForm = ({ onMatchCreated }) => {
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        const res = await axios.get("http://localhost:3026/api/players", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setRegisteredPlayers(res.data.players || []);
      } catch (err) {
...
const handleSubmit = async (e) => {
  ...
  const matchData = {
    ...
    status,
    date: new Date(),
    createdBy: localStorage.getItem("userId"), // ✅ Add this line
    inningsScores: [], // ✅ this line fixes the Joi error
...
//           <PlayerSelect
//             label=""
//             players={registeredPlayers.filter(p => !team1Players.includes(p._id))}
//             isMulti
//             value={team2Players}
//             onChange={setTeam2Players}
//             placeholder="Select players from registered list"
