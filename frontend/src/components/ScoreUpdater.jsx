...
//   const currentTeamPlayers = match?.teams?.[currentTeamIndex]?.players || [];
...
//         `http://localhost:3026/matches/${match._id}/ball`,
//         {
//           strikerId,
//           nonStrikerId,
//           bowlerId,
...
//       if (res.data.msg === "Match completed") {
//         alert("Match is over");
//       }
...
//       <h3 className="text-lg font-bold mb-4">🎯 Update Ball</h3>
...
//       <PlayerSelect
//         label="Bowler"
//         players={match?.teams?.[(currentTeamIndex + 1) % 2]?.players || []}
//         value={bowlerId}
...
//         const opponentIds = match?.teams?.[(currentTeamIndex + 1) % 2]?.players || [];

//         const res = await axios.post("http://localhost:3026/api/players/bulk", {
...
//       const res = await axios.patch(
//         `http://localhost:3026/matches/${match._id}/ball`,
//         { strikerId, nonStrikerId, bowlerId, runs, isWicket },
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
...
//       if (teamIds.length === 0 || opponentIds.length === 0) return;

//       try {
//         const res = await axios.post(
//           "http://localhost:3026/api/players/bulk",
//           { ids: [...teamIds, ...opponentIds] },
//           { headers: { Authorization: `Bearer ${token}` } }
//         );
...
//     fetchPlayers();
//   }, [match._id, match.currentScore.team]);
...
//       <PlayerSelect label="Striker" players={teamPlayers} value={strikerId} onChange={setStrikerId} />
//       <PlayerSelect label="Non-Striker" players={teamPlayers} value={nonStrikerId} onChange={setNonStrikerId} />
//       <PlayerSelect label="Bowler" players={opponentPlayers} value={bowlerId} onChange={setBowlerId} />
