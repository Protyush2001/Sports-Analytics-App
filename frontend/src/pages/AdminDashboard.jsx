...
const [showMatches, setShowMatches] = useState(false);
  ...
  try {
    await axios.patch(`http://localhost:3026/admin/user/${userId}`, { role: newRole }, {
      headers: { Authorization: `Bearer ${token}` },
    });
    alert("User role updated successfully");
    // Refresh user list
    const res = await axios.get("http://localhost:3026/admin/users", {
      headers: { Authorization: `Bearer ${token}` },
    });
    setUsers(res.data);
  } catch (err) {
...
const handleControlMatch = async (matchId) => {
  ...
  if (action === "delete") {
    try {
      await axios.delete(`http://localhost:3026/admin/matches/${matchId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      alert("Match deleted");
      const res = await axios.get("http://localhost:3026/admin/matches", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setMatches(res.data);
    } catch (err) {
    ...
    if (!newStatus) return;
    try {
      await axios.patch(`http://localhost:3026/matches/${matchId}`, { status: newStatus }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await axios.get("http://localhost:3026/admin/stats", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setStats(res.data);
      } catch (err) {
      ...
      try {
        const [userRes, matchRes] = await Promise.all([
          axios.get("http://localhost:3026/admin/users", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("http://localhost:3026/admin/matches", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
